...
Passwordless.prototype.acceptToken = function(options) {
  ...
  return function(req, res, next) {
    ...
    const tokenField = options.tokenField ? options.tokenField : 'token'
    const uidField = options.uidField ? options.uidField : 'uid'

    let token = req.query[tokenField],
    ...
    if (token && uid) {
      self._tokenStore.authenticate(token, uid.toString(), function(
        error,
        valid,
        referrer,
      ) {
  ...
  return function(req, res, next) {
    ...
    if (options.failureRedirect) {
      ...
      if (options.originField) {
        let parsedRedirectUrl = url.parse(options.failureRedirect),
          queryParam = '?'
...
Passwordless.prototype.requestToken = function(getUserID, options) {
  ...
  return function(req, res, next) {
    const sendError = function(statusCode, authenticate) {
      if (options.failureRedirect) {
        if (options.failureFlash) {
          req.flash('passwordless', options.failureFlash)
        }
    ...
    const userField = options.userField ? options.userField : 'user'
    const deliveryField = options.deliveryField
      ? options.deliveryField
      : 'delivery'
    const originField = options.originField ? options.originField : null

    let user, delivery, origin
    ...
    let deliveryMethod = self._defaultDelivery
        ...
        if (uidError) {
          ...
          try {
            if (
              ...
              token = (deliveryMethod.options.tokenAlgorithm ||
                self._generateToken())()
            }
          } catch (err) {
...
Passwordless.prototype.addDelivery = function(name, sendToken, options) {
  ...
  if (
    ...
    (options.numberToken &&
      (!options.numberToken.max || typeof options.numberToken.max !== 'number'))
  ) {
 ...
 * @private
 */
Passwordless.prototype._redirectWithSessionSave = function(
  req,
  res,
  next,
  target,
...
Passwordless.prototype._generateToken = function(randomBytes) {
  ...
  return function() {
    const buf = crypto.randomBytes(randomBytes)
